...
dataset = pd.read_excel("https://github.com/gunalan-samsil/adaptiv-effort-estimator/blob/master/dataset_integration_v2.xlsx?raw=true",skip_header=1)
Y=dataset.iloc[:, 13:]
X=dataset.iloc[:,1:13]
header=list(X)
imputer = Imputer()
...
def intRegression(req):
    ...
    op_lrt=loaded_model.predict(ds)
    print(op_lrt)
...
def retrain():
    ...
    try:
        ...
        df = pd.DataFrame(op)
        pd.to_excel("https://github.com/gunalan-samsil/adaptiv-effort-estimator/blob/master/dataset_integration_v2.xlsx?raw=true",skip_header=1)
